 ...
 */
export type FunctionEventListener = (this: Framecast, ...args: any[]) => Promise<any>;
...
export class Framecast {
  ...
  on(eventType: keyof ListenerMap, listener: Function): void {
    ...
    if (eventType.startsWith('function:') && this.listeners[eventType].size > 0) {
   ...
   */
  async call<ReturnValue = any>(functionName: string, ...args: any[]): Promise<ReturnValue> {
    const id = nanoid();
    ...
    return new Promise((resolve, reject) => {
      const timeout = window.setTimeout(() => {
        ...
        reject(new Error(`${functionName} timed out after ${this.config.functionTimeoutMs}ms`));
   ...
   */
  async evaluate<ReturnValue = any>(function_: (...args: any[]) => ReturnValue, ...args: any[]): Promise<ReturnValue> {
    const functionString = function_.toString();
    const argsString = args.map(a => JSON.stringify(a)).join(',');
    const calledFunctionString = `(${functionString})(${argsString})`;

    return this.call('evaluate', calledFunctionString);
  }
  ...
  state<StateType = any>(key: string, initialValue: StateType): WritableAtom<StateType> {
    ...
    this.call<StateType>(`state:get:${key}`)
      .then(value => {
        ...
        if (isInitialValue && value !== undefined) {
          $atom.set(value);
        }
  ...
  private async handleBroadcast(data: any) {
    for (const listener of this.listeners['broadcast'] ?? []) {
      Reflect.apply(listener as Function, this, [data]);
    }
  }
   ...
   */
  private async handleFunctionCall(eventType: keyof ListenerMap, id: number, args: any[]) {
   ...
   */
  private async handleFunctionResult(data: { type: 'functionResult'; id: string; result?: any; error?: Error }) {
...
function isStateSyncMessage(message: any): message is { type: 'state:sync'; key: string; value: any } {
  if (!message) {
    return false;
  }
  ...
  return 'key' in message && 'value' in message && 'type' in message && message.type === 'state:sync';
}
