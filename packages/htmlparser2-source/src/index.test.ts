...
`;
  ...
  test('should correctly calculate endIndex for attribute with empty string value (hidden="")', () => {
    const source = `<div id="test" hidden="" class="test">content</div>`;
    const document = parseDocument(source);
    const [element] = document.children;
    ...
    // Second attribute: hidden="" (with empty string value)
    expect(element.source.attributes[1].name.data).toBe('hidden');
    expect(element.source.attributes[1].value).not.toBe(null);
    expect(element.source.attributes[1].value!.data).toBe('');
  ...
  test('should correctly calculate endIndex for attribute with quoted value (hidden="true")', () => {
    const source = `<div id="test" hidden="true" class="test">content</div>`;
    const document = parseDocument(source);
    const [element] = document.children;
    ...
    // Second attribute: hidden="true" (with quoted value)
    expect(element.source.attributes[1].name.data).toBe('hidden');
    expect(element.source.attributes[1].value).not.toBe(null);
    expect(element.source.attributes[1].value!.data).toBe('true');
    ...
    // Verify there's a space between hidden="true" and class
    const spaceBetween = source.slice(
      element.source.attributes[1].source.endIndex + 1,
      element.source.attributes[2].source.startIndex
    );
