 ...
 */
export function parseHtmlAtPosition(
  html: string,
  ...
  for (const node of nodes) {
    ...
    if (isTag(node)) {
      ...
      if (node.source?.attributes) {
        for (const attr of node.source.attributes) {
          if (attr.name) {
            attr.name.startIndex += offset;
            attr.name.endIndex += offset;
          }
          if (attr.value) {
            attr.value.startIndex += offset;
            attr.value.endIndex += offset;
          }
          if (attr.source) {
            attr.source.startIndex += offset;
            attr.source.endIndex += offset;
          }
        }
      }
